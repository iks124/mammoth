...
def register_hooks(
    ...
    forward=True,
    backward=True,
    forward_hooks_dict=None,
    bacward_hooks_dict=None,
):
    module.name = name
    if forward:
        ...
        if "lin_proj" in name:
            ...
            module.forward_handle = module.register_forward_hook(
                forward_hooks_dict["hook_forward"]
            )
...
class KFACComputer(nn.Module):
    ...
    def to_be_fishered(self, name, module, all_param_finetuned):
        if (
            ...
            return False
        return (
            f"{name}.weight" in all_param_finetuned
    ...
    def compute(self, net, head, delta_w_names, dataset, use_head=False):
        ...
        for name, module in net.visual_encoder.named_modules():
            if self.to_be_fishered(name, module, all_param_finetuned):
                module.compute_bias = f"{name}.bias" in all_param_finetuned
        ...
        fake_optim = torch.optim.SGD(
            params=[
                p
                for (n, p) in net.visual_encoder.named_parameters()
                if n in delta_w_names
            ],
            lr=0.0,
        )
        ...
        for name, module in net.visual_encoder.named_modules():
            ...
            if (
                "cls_token" in name
                and "cls_token_layer.class_embedding" in all_param_finetuned
            ):
                register_hooks(
                    name,
                    module,
        ...
        fake_param = torch.tensor([1.0], requires_grad=True).to(self.device)
