...
import torch
from models.tak_v2_utils.backbone import Backbone, create_clip
from models.tak_v2_utils.backbone import build_classification_head
from models.tak_v2_utils.utils import FisherLoader
from models.tak_v2_utils.utils import OptimizerBuilder
from models.tak_v2_utils.utils import compute_acc_on_last_task

from models.tak_v2_utils.fisher_kfac import KFACComputer

from models.tak_v2_utils.merging import get_merging_function
from models.tak_v2_utils.utils import get_parameter
from utils.evaluate import evaluate 

import wandb


class TAKv2(ContinualModel):
    ...
    def get_parser(parser) -> ArgumentParser:
        ...
        parser.add_argument('--merging', type=str, default='ta', choices=["ta", "dare", "iso", "ties", "tsv"], help="Merging strategy for task vectors")
        parser.add_argument('--alpha_merging', type=str, default='one', help="Alpha used during merge."
        ...
        parser.add_argument('--save_task_vectors',  type=binary_to_boolean_type, default=0, help="Save computed task vectors?")
        parser.add_argument('--virtual_bs_n', type=int, default=1, help='chose how many chunks for vitual batch size')
        ...
        parser.add_argument('--reg_lambda', type=float, default=500, help='Regularization weight (lambda in the paper)')
        parser.add_argument('--fisher_ft_proj_scaler', type=float, default=0.1, help="Regularization scaling coeff. for the final linear projection")
        parser.add_argument('--fisher_norm_scaler', type=float, default=10, help="Regularization scaling coeff. for inner feed-forward layers")
        parser.add_argument('--scheduler_ntk', type=str, default="cosine", choices=["none", "cosine", "cosine_plus", "decay", "step"], help="LR scheduler type")
        parser.add_argument('--clip_grad_norm', type=float, default=None, required=False, help="Gradient clipping norm value - used if >0 and not None")

        # KFAC approximation args
        parser.add_argument('--load_fisher', type=binary_to_boolean_type, default=0, help='Load KFAC map from cache?')
        parser.add_argument('--fisher_cache', type=str, default='fisher_cache', help='Path on which to save or load KFAC maps')
        ...
        parser.add_argument('--fisher_task_id', type=int, default=None, required=False, help='Compute KFAC approx. on this specific task')
        parser.add_argument('--fisher_ideal', type=binary_to_boolean_type, default=0, help="Keep and use the fisher of each task (ideal - Eq. 7) or just the accumulated one (Eq. 8)")
        parser.add_argument('--fisher_num_samples_expectation', type=int, default=0, help="Compute KFAC approx. on a fixed number of samples, subset of all the task")

        # TODO: create a new param group for ablation arguments
        parser.add_argument('--tangent',  type=binary_to_boolean_type, default=1, help="Use or disable linearized training and inference (NTK regime)")


        # TODO: create a new param group for extra parameters
        parser.add_argument('--use_lora', type=binary_to_boolean_type, default=0)
        parser.add_argument('--fp_precision', type=str, default='fp32', choices=['fp32', 'fp64'], help="Floating point fp_precision used during KFAC computations")
        parser.add_argument('--resume', type=binary_to_boolean_type, default=0, help="Resume previous training? NOTE: requires `load_path`")
        parser.add_argument('--load_path', type=str, default=None, required=False, help="Path from which load the previous task's task vectors. Used with `resume=1`")

        # TODO: create group for evaluation args
        parser.add_argument('--alpha_sweep_start', type=float, default=0.1, help="Starting merging alpha value for sensitivity analysis - used by `compute_metrics_by_alpha`")
        parser.add_argument('--alpha_sweep_end', type=float, default=1.5, help="Final merging alpha value for sensitivity analysis - used by `compute_metrics_by_alpha`")
        parser.add_argument('--alpha_sweep_step', type=float, default=0.1, help="Step alpha value for sensitivity analysis - used by `compute_metrics_by_alpha`")
    ...
    def __init__(self, backbone, loss, args, transform, dataset):
        ...
        if self.args.save_task_vectors:
            ...
            tv_path = os.path.join(self.args.checkpoint_path, f"{self.args.conf_jobnum}_{dataset.NAME}_args.json")
        ...
        if not self.args.load_fisher:
            if self.args.fisher_task_id is None or self.current_task == self.args.fisher_task_id:
                ...
                ggT, aaT, ffT, num_ggT, num_aaT = \
                    self.fisher_computer.compute(self.net, None, self.delta_w_names, dataset, use_head=False)
            ...
            counts = [self.fisher_loader.load_kfac(t, only_counts=True) for t in range(dataset.N_TASKS)]
    ...
    def end_task(self, dataset: ContinualDataset) -> None: #TODO  set the model in eval mode
        ...
        if self.args.save_task_vectors:
            ...
            base_path = os.path.join(self.args.checkpoint_path, self.args.fisher_cache,f"{self.args.conf_jobnum}_{dataset.NAME}_task_{self.current_task}")
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if self.args.tangent:
            ...
            image_features, jvp = func.jvp(forward_fun, (tuple(params),), (tuple(self.get_all_parameters_from_dict()),),) 
        ...
        if (self.args.load_fisher) and (self.task_iteration > 0) and (self.task_iteration % self.args.virtual_bs_n == 0):
        ...
        if (self.task_iteration > 0) and self.task_iteration % self.args.virtual_bs_n == 0:
            if self.scheduler1:
    ...
    def forward(self, x):
        ...
        if self.args.tangent:
            ...
            params = [param for name, param in self.net.visual_encoder.named_parameters() if name in self.delta_w_names]
            image_features, jvp = func.jvp(forward_fun, (tuple(params),),  
            ...
            image_features = func.functional_call(self.net.visual_encoder, dict_param, x) 
            image_features = nn.functional.normalize(image_features, dim=-1)
    ...
    def compute_metrics_by_alpha(self):
        ...
        alphas = np.arange(self.args.alpha_sweep_start, self.args.alpha_sweep_end + self.args.alpha_sweep_step, self.args.alpha_sweep_step).tolist()
